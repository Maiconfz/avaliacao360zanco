...
import com.avaliacao360zanco.service.EquipeService;
...
public class EquipeResource {
	 ...
	 *
	 * @param equipe
	 *            the equipe to create
	 * @return the ResponseEntity with status 201 (Created) and with body the
	 *         new equipe, or with status 400 (Bad Request) if the equipe has
	 *         already an ID
	...
	public ResponseEntity<Equipe> createEquipe(@Valid @RequestBody Equipe equipe) throws URISyntaxException {
		...
		if (equipe.getId() != null) {
			return ResponseEntity.badRequest().headers(
					HeaderUtil.createFailureAlert("equipe", "idexists", "A new equipe cannot already have an ID"))
					.body(null);
		}
		Equipe result = equipeService.save(equipe);
		return ResponseEntity.created(new URI("/api/equipes/" + result.getId()))
				.headers(HeaderUtil.createEntityCreationAlert("equipe", result.getId().toString())).body(result);
	}
	 ...
	 *
	 * @param equipe
	 *            the equipe to update
	 * @return the ResponseEntity with status 200 (OK) and with body the updated
	 *         equipe, or with status 400 (Bad Request) if the equipe is not
	 *         valid, or with status 500 (Internal Server Error) if the equipe
	 *         couldnt be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<Equipe> updateEquipe(@Valid @RequestBody Equipe equipe) throws URISyntaxException {
		...
		Equipe result = equipeService.save(equipe);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert("equipe", equipe.getId().toString()))
				.body(result);
	}
	...
	@GetMapping("/equipes")
	@Timed
	public ResponseEntity<List<Equipe>> getAllEquipes(@ApiParam Pageable pageable) throws URISyntaxException {
	 ...
	 *
	 * @param id
	 *            the id of the equipe to retrieve
	 * @return the ResponseEntity with status 200 (OK) and with body the equipe,
	...
	public ResponseEntity<Equipe> getEquipe(@PathVariable Long id) {
		...
		return Optional.ofNullable(equipe).map(result -> new ResponseEntity<>(result, HttpStatus.OK))
	 ...
	 *
	 * @param id
	 *            the id of the equipe to delete
